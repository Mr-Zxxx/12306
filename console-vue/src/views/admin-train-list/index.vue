...
const handleBook = (record) => {
    window.open(
        `buyTicket?trainNumber=${record.trainNumber}&&trainId=${record.trainId
        }&&${Object.entries(headSearch)
            ?.map((item) => {
                return `${item[0]}=${item[0] === 'departureDate' ? item[1].format('YYYY-MM-DD') : item[1]
                    }`
            })
            .join('&&')}`
    )
        ...
        <Space direction="vertical" size="middle" :style="{ width: '100%' }">
            ...
            <Form layout="inline" :colon="false" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }">
                <Space direction="vertical" size="middle" :style="{ width: '100%' }">
                    <Card :style="{ width: '100%' }">
                        <Row class="row" :style="{ maxWidth: '1150px' }">
                            <Col :span="5" :offset="0">
                            <FormItem label="出发地">
                                <Select v-model:value="headSearch.fromStation" :style="{ width: '150px' }"
                                    :show-arrow="false" :show-search="true" :options="state.stationList.map((item) => ({
                                        label: item.name,
                                        value: item.code
                                    }))">
                                </Select>
                            ...
                            <div :style="{
                                ...
                                <RetweetOutlined @click="exchangeCity"
                                    :style="{ cursor: 'pointer', fontSize: '20px', color: '#1990ff' }" />
                            </div>
                            </Col>
                            <Col :span="5">
                            <FormItem label="目的地">
                                <Select v-model:value="headSearch.toStation" :style="{ width: '150px' }"
                                    :show-arrow="false" :show-search="true" :options="state.stationList.map((item) => ({
                                        label: item.name,
                                        value: item.code
                                    }))">
                            ...
                            <Col :span="5">
                            <FormItem label="车次号">
                                <a-input v-model:value="headSearch.trainNumber" :style="{ width: '150px' }"
                                    placeholder="请输入车次号"></a-input>
                                <!-- <Select v-model:value="headSearch.fromStation" :style="{ width: '150px' }"
                                    :show-arrow="false" :show-search="true" :options="state.stationList.map((item) => ({
                                        label: item.name,
                                        value: item.code
                            ...
                            <Col :span="5">
                            <FormItem label="出发日">
                                <DatePicker v-model:value="headSearch.departureDate" :style="{ width: '150px' }"
                                    :show-arrow="false" :allow-clear="false">
                                </DatePicker>
                            </FormItem>
                            </Col>
                            <Col span="2">
            ...
            <Row>
                ...
                <span class="time-title">{{
                    ...
                    }}</span>） 共计<span :style="{ fontWeight: 'bolder' }">{{
            ...
            <div class="custome-table">
                <Table :columns="columns" :data-source="state.trainList" :pagination="false" :scroll="{ x: 1350 }">
                    ...
                    <template #trainNumber="{ text, record }">
                        <h1 class="card-name" :style="{ cursor: 'pointer' }" @click="() => {
                            ...
                            <Tooltip :get-popup-container="(node) => node.parentNode" placement="rightTop"
                                trigger="click">
                                <div>
                                    <!-- 车次名称 -->
                                    {{ text }}
                    ...
                    justify-content: center;">
                                    <div v-for="tag in record?.trainTags" :key="tag" :style="{
                                        color: TRAIN_TAG.find((item) => item.value === tag)
                                            ?.color,
                                        border: `1px solid ${TRAIN_TAG.find((item) => item.value === tag)?.color
                                            }`,
                                        padding: '2px',
                                        aspectRatio: '1 / 1',
                                        borderRadius: '2px',
                                ...
                                <template #title>
                                    <!-- 线路表 -->
                                    <Table :columns="cardInfoColumns" :data-source="state.trainStationList"
                                        :pagination="false" :loading="state.loading"></Table>
                                </template>
                            </Tooltip>
                        </h1>
                    </template>
                    <template #station="{ _, record }">
                        <div>
                            <span :style="{
                                ...
                                fontWeight: 'bolder',
                                marginRight: '2px'
                            }">{{ record.departureFlag ? '始' : '过' }}</span>{{ record.departure }}
                        </div>
                        <div>
                            <span :style="{
                                display: 'inline-block',
                                ...
                                fontWeight: 'bolder',
                                marginRight: '2px'
                            }">{{ record.arrivalFlag ? '终' : '过' }}</span>{{ record.arrival }}
                        </div>
                    </template>
